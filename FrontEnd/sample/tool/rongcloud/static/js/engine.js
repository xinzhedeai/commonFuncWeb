...
$(function (undefined) {
    ...
    //好友列表
    $scope.friendsList = [{"id":"585c3527-ae9b-11e5-bbdb-00163e024a0c","username":"6shen","portrait":"http://img4.duitang.com/uploads/item/201406/07/20140607182836_8MEhe.jpeg"},
                          {"id":"1da17548-94d8-11e5-bbdb-00163e024a0c","username":"afu的古事","portrait":"http://img4.duitang.com/uploads/item/201406/07/20140607182836_8MEhe.jpeg"}
                          ];
    //会话标题
...
function initConversationList() {
    ...
    $scope.ConversationList.forEach(function (item) {
    	var person = $scope.friendsList.filter(function (fitem) {
            return fitem.id == item.getTargetId();
        })[0];
    	var portrait = '/storage/upload/head/' + person.portrait;
        _html += String.stringFormat(conversationStr, item.getConversationType().valueOf(), item.getTargetId(), item.getConversationTitle(), portrait, item.getUnreadMessageCount() == 0 ? "hidden" : "", item.getUnreadMessageCount(), item.getConversationTitle(), new Date(+item.getLatestTime()).toString().split(" ")[4]);
    });
...
function getHistory(id, name, type, again) {
    ...
    $scope.historyMessages.forEach(function (item) {
        _html += String.stringFormat(historyStr, item.getMessageDirection() == 0 ? "other_user" : "self", item.getMessageDirection() == 1 ? "/storage/upload/head/"+owner.portrait : "static/images/personPhoto.png", "", item.getMessageDirection() == 0 ? 'white_arrow.png' : 'blue_arrow.png', myUtil.msgType(item), item.getMessageId());
    });
    if (again == 1 && _html) {
...
function connect(token){
    //初始化SDK
    RongIMClient.init("k51hidwq18o4b"); //e0x9wycfx7flq z3v5yqkbv8v30
	//链接融云
//	var token = 'AumyQNkWWzCiX2xRxjwJdNCW1zXmYI2s01Ez4QtPHB6cbT4Kh+ua7/NynNo2XNDhbBpYwWL5vfAjVNGd5IAHlhPeQa/U+neey4uSHGX1/GsFqI0RonpRntFuhxOl+PtlyupqzdDDLA0=';
...
//                audio.play();
            ...
            if (tempval.getConversationTitle() == undefined || tempval.getConversationTitle() == '') {
...
var $engine = {
		token : null,
		getToken : function(){
			var getToken = new nssoft.openApi.getToken.api();
			getToken.request({
				method : 'POST',
				async : false,
				url : '/v1/api/rongcloud/getToken.json'
			});
		},
		getFriends : function(){
			var getFriends = new nssoft.openApi.getFriends.api();
			getFriends.request({
				method : 'POST',
				async : false,
				url : '/v1/api/talk/getFriends.json'
			});
		}
};

(function() {//getToken
	$class('nssoft.openApi.getToken.api').extend(nssoft.absAPI).define({
		onSuccess : function(status, res) {
			if (res.errCd === 0) {
				var result = res.result;
				if (result) {
					$engine.token = result.token;	
					owner.name = result.name;
					owner.id = result.userId;
					owner.portrait = "/storage/upload/head/"+result.portrait;
					$("img[RCTarget='owner.portrait']").attr("src", owner.portrait);
				    $('span[RCTarget="owner.name"]').html(owner.name);
					connect($engine.token);
				} else {
					var txt = res.errMsg;
...
(function() {//getFriends
	$class('nssoft.openApi.getFriends.api').extend(nssoft.absAPI).define({
		onSuccess : function(status, res) {
			if (res.errCd === 0) {
				var result = res.result;
				if(result){
					$scope.friendsList = result.rows;
				    var htmltest = '';
				    var path = '/storage/upload/head/';
				    $scope.friendsList.forEach(function (item) {
				    	htmltest += String.stringFormat(friendListStr, item.id, item.username, path+item.portrait);
				    });
				    $("#friendsList").html(htmltest)
			}else{
				var txt = res.errMsg;
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
			}
