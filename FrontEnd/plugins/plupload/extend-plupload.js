					...
					var options_extend = {
						...
						init: {
							/*BeforeUpload: function(uploader,file){
								var Tsize=plupload.formatSize(file.size);
							},
							PostInit: function() {
								document.getElementById('modalButton').onclick = function() {
									var Tsize=plupload.formatSize(file.size);
									if(Tsize.substring(0,Tsize.length-2)>1024){
										alert("图片过大请重新选择");
									}else{
										uploader.start();
										return false;
							...
							FileUploaded: function(uploader, file, responseObject) {
								...
								if (res.errCd === 0) {
									...
									if (result) {
									/*	window.imgMulti.push(result.FILE_PATH);
										$(".imgMulti").val(window.imgMulti);*/
										var files = result.files;
										var img=$("<img src='"+result.FILE_PATH+"' alt='' width='198' height='198'/>");
										var leng=$(".gbin1-list li").length;
										for(var i=leng-1;i>=0;i--){
											var sty=$(".gbin1-list li").eq(i).find('img').attr('src');
											if(sty){
												
											}else{
												$(".gbin1-list li").eq(i).find('.item').append(img);
											}
										}
								...
								var displayMsg = msg, 
									tooltipId = Math.round(Math.random() * 100000);
								...
								plupload_header.append('' + 
										'<div class="plupload_message ui-state-' + uistate + '">' +
											...
											'<p style="text-align:center;">' +
												'<strong style="word-break: break-all;font-size:15px;">' + displayMsg + '</strong><br/>' +
												'<i>文件: ' + file.name + '</i>' +
											'</p>' +
									'</div>');
								if (displayMsg.length > 110) {
						...
						init: {
							...
							FileUploaded: function(uploader, file, responseObject) {
								...
								if (res.errCd === 0) {
									var result = res.result;
									if (result) {
										$(".cerBox").find('img').remove();
										var nimg=$("<img src='"+result.FILE_PATH+"' alt='' width='160' height='160' style='margin:0'>");
										$(".cerBox").append(nimg);
										msg = result.msg;
									}
								} else {
