(function($) {
    ...
    var defaulOption = {
      ...
      var showClickTextFunction = function() {
        ...
        $(".textAreaCount").text($("#content").val().length+"/200");
      ...
      var _initItems = function(items) {
        ...
        for (var i = 0; i < items.length; i++) {
            if(items[i].Nodetitle > maxFontNumber){
              maxFontNumber = items[i].Nodetitle.length;
              }
          var suggestItem = $('<div></div>'); //创建一个子<div>
          suggestItem.attr('id', items[i].NodeID);
          suggestItem.append(items[i].Nodetitle);
          suggestItem.css({
